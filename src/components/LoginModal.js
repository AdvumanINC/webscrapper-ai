...
function LoginModal({ onClose, onLogin }) {
  ...
  return (
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem 2rem' }}>
          <div style={{ width: '100%', maxWidth: '28rem' }}>
            ...
            <div style={{ marginBottom: '2rem' }}>
              <h1 style={{ fontSize: '1.875rem', fontWeight: 600, color: '#fff', marginBottom: '0.5rem' }}>Welcome back</h1>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <a href="#" style={{ fontSize: '0.875rem', color: '#9ca3af', textDecoration: 'none', transition: 'color 0.2s' }} onMouseEnter={(e) => e.target.style.color = '#fff'} onMouseLeave={(e) => e.target.style.color = '#9ca3af'}>
                  Forgot password?
                </a>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                style={{ width: '100%', padding: '0.75rem 1rem', backgroundColor: loading ? '#555' : '#16a34a', color: '#fff', border: 'none', borderRadius: '0.5rem', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 500, fontSize: '0.95rem', transition: 'background-color 0.2s', opacity: loading ? 0.5 : 1 }}
    ...
    {showSignup && <SignupModal onClose={onClose} onSignup={onLogin} />}
    </>
  );
}
